      ...
      <AnimatePresence mode="wait">
        {confirmModal && (
          <Modal title="Xác nhận chuyển khoản" className="w-[320px]" onClose={() => { setOpenConfirmModal(false); }}>
            <div>
              <header>
                Để shop có thể xác minh việc chuyển khoản, xin vui lòng ghi chú <b>5 số cuối</b> trên số tài khoản bạn đã sử dụng cho việc chuyển khoản để shop kiểm chứng lại
              </header>
              <input type="text" pattern="[0-9]" maxLength={5} className="my-1 h-[50px] p-1 text-2xl font-semibold text-center rounded-sm w-full" onInput={onInputted} />
            </div>
